...
/**
 * 地理空間索引（沿用 RoboTaxi ⑤ 的做法）
 * --------------------------------------------------
 * 對應真實元件：Redis GEO / S2 / QuadTree。
 * 在外送平台裡，派單時要在「附近可用外送員」中挑最近的指派；
 * 核心任務：在大量外送員中毫秒級找出「某座標（餐廳）附近 / 最近 K 個可用外送員」。
 *
 * 本 demo 採教學版：
...
final class GeoIndex
{
    ...
    /**
     * 查附近可用外送員（半徑內，依距離由近到遠）。
     * @return list<array{courier:array, distance_km:float}>
     */
    public function nearby(float $lat, float $lng, float $radiusKm, int $limit = 20): array
    {
        $results = [];
        foreach ($this->store->allCouriers() as $c) {
            if (($c['status'] ?? '') !== 'available') {
                continue; // 只看可用外送員（送單中 / 離線不納入）
            }
            $dist = self::haversine($lat, $lng, (float)$c['lat'], (float)$c['lng']);
            if ($dist <= $radiusKm) {
                $results[] = ['courier' => $c, 'distance_km' => round($dist, 3)];
            }
        }
    ...
    /**
     * 最近 K 個可用外送員（派單引擎用：拿到一串按距離排序的候選，
     * 逐一嘗試原子指派，第一個指派成功者即派單結果）。
     * @return list<array{courier:array, distance_km:float}>
     */
    ...
    /** haversine 球面距離（公里）——粗篩後的精算與排序依據，也用來估 ETA */
